    ...
    <Link to="/talks/$slug" params={{ slug: talk.slug }} className="group relative block">
      ...
      >
            ...
            <div className="flex flex-wrap gap-2">
              {talk.topics.slice(0, 2).map((topic) => (
                <Badge
                  key={topic}
                  variant="outline"
                  className="border-gold/30 bg-gold/15 text-gold tracking-wide uppercase"
                >
                  {topic}
                </Badge>
              ))}
        ...
        <div className="absolute top-4 right-4">
          <div className="bg-gold/10 border-gold/20 flex size-8 items-center justify-center rounded-full border">
