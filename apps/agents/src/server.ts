      ...
      tools: {
        ...
        scheduleTask: tool({
          ...
          execute: async ({ when, description }) => {
            ...
            if (scheduleResult.isErr()) {
              ...
              return `Error scheduling task: ${error.message}`;
            }
        ...
        cancelScheduledTask: tool({
          ...
          execute: async ({ taskId }) => {
            const cancelResult = await Result.tryPromise(() => this.cancelSchedule(taskId));

            if (cancelResult.isErr()) {
              const error = cancelResult.error;
              return `Error cancelling task: ${error.message}`;
            }
