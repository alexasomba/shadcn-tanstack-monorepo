...
function useMultibandVolume(
  ...
  const [frequencyBands, setFrequencyBands] = React.useState<number[]>(
    () => Array.from({ length: bands }).fill(0) as number[],
  );
  const bandsRef = React.useRef<number[]>(Array.from({ length: bands }).fill(0) as number[]);
  const frameId = React.useRef<number | undefined>(undefined);

  React.useEffect(() => {
    if (!mediaStream) {
      const emptyBands = Array.from({ length: bands }).fill(0) as number[];
...
>(
  (
    ...
    const fakeVolumeBandsRef = React.useRef<number[]>(
      Array.from({ length: barCount }).fill(0.2) as number[],
    );
    ...
    React.useEffect(() => {
      ...
      if (state !== "speaking" && state !== "listening") {
        const bands = Array.from({ length: barCount }).fill(0.2) as number[];
