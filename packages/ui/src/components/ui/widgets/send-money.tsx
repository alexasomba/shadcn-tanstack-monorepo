...
const VisaIcon = () => (
  <span className="text-sm font-semibold text-neutral-900 italic dark:text-white">VISA</span>
);
...
const InputField = ({
  ...
  <div className="mb-4">
    <label className="mb-1 block text-sm text-neutral-500 dark:text-neutral-400">{label}</label>
    <input
      value={value}
    ...
    <motion.div>
      ...
      >
        <div className="mb-4 flex flex-wrap items-center justify-between gap-2">
          <span className="text-sm text-neutral-500 dark:text-neutral-400">Available Cards</span>
    ...
    <motion.div>
      ...
      >
        <div className="mb-5 rounded-2xl border border-neutral-200 bg-neutral-100 p-4 dark:border-neutral-700 dark:bg-neutral-800">
          <p className="mb-1 text-sm text-neutral-500 dark:text-neutral-400">Total Balance</p>
          <h3 className="text-2xl font-semibold text-neutral-900 dark:text-white">$12,450.00</h3>
        </div>
      ...
      >
        <AnimatePresence mode="wait">
          {!view ? (
              ...
              <div className="space-y-2">
                {[
                  ...
                  <button
                    ...
                    className="flex w-full items-start gap-3 rounded-2xl p-3 transition hover:bg-neutral-100 sm:items-center sm:gap-4 sm:p-4 dark:hover:bg-neutral-800"
                  >
                    <motion.div
                      layoutId={`icon-${opt.id}`}
                      className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl border border-neutral-200 bg-neutral-100 text-neutral-400 sm:h-12 sm:w-12 dark:border-neutral-700 dark:bg-neutral-800 dark:text-neutral-400"
                    >
                    ...
                    <div className="text-left">
                      ...
                      <p className="text-sm text-neutral-500 dark:text-neutral-400">{opt.sub}</p>
                    </div>
                  </button>
                ))}
              </div>
            ...
            <>
              {view === "bank" && (
                <BankTransferView onClose={() => setView(null)} onProceed={onProceed} />
              )}
              {view === "card" && (
                <CardView cards={cards} onClose={() => setView(null)} onProceed={onProceed} />
              )}
              {view === "wallet" && (
                <WalletView onClose={() => setView(null)} onProceed={onProceed} />
              )}
            </>
          )}
        </AnimatePresence>
