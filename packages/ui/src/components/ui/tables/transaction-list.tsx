            ...
            {isOpen ? (
              ...
              >
                ...
                {transactions.map((item) => (
                  <TransactionItem key={item.id} data={item} onClick={() => setOpen(item.id)} />
                ))}
              ...
              selected && (
                <motion.div exit={{ opacity: 0 }}>
                  <TransactionItemExpanded data={selected} onClose={() => setOpen(null)} />
                </motion.div>
              )
            )}
          </AnimatePresence>
...
function TransactionItemExpanded({ data, onClose }: { data: Transaction; onClose: () => void }) {
  return (
