# Sentry Observability Integration Design and Verification

## Overview

This analysis covers the integration and design of integration tests for Sentry observability in the `apps/data-service` Cloudflare Worker. The objective of Milestone 6 (R5) is to verify that unhandled exceptions are captured by Sentry with accurate contextual tags across the four primary entry points/components of the data service:

1. **Hono HTTP Router**: Standard HTTP routing and API endpoints.
2. **Durable Object Workflows**: Asynchronous step execution orchestration.
3. **Queue Consumers**: Outbox message draining and background task queue jobs.
4. **Cron Scheduled Tasks**: Execution of periodic maintenance jobs (e.g. outbox draining tick).

---

## 1. Exception Capture Failure Paths & Tag Schema

| Component            | Error Boundary Location                                      | Trigger / Test Method                                                         | Captured Tags                       |
| -------------------- | ------------------------------------------------------------ | ----------------------------------------------------------------------------- | ----------------------------------- |
| **Hono HTTP Router** | `app.onError` in `src/index.ts`                              | Request to `/api/debug/sentry-test` (throws `Error("Sentry test exception")`) | None (General HTTP exception error) |
| **Workflows**        | `crashWorkflowHandler` in `src/endpoints/workflows/crash.ts` | POST `/workflows/instances/{id}/crash`                                        | `workflowInstanceId: string`        |
| **Queue Consumers**  | `handleJobsBatch` catch-block in `src/jobs/queue.ts`         | Invoking `worker.queue(...)` with a failing outbox drain message              | `jobType: string`, `jobId: string`  |
| **Cron Tasks**       | `cronTask` catch-block in `src/jobs/cron.ts`                 | Invoking `worker.scheduled(...)` with a failing cron task function            | `cronTask: string`                  |

---

## 2. Proposed Source Code Enhancements

### A. Queue/Outbox Job Sentry Integration

Currently, `handleJobsBatch` in `apps/data-service/src/jobs/queue.ts` catches message processing errors but does not report them to Sentry.

We propose adding the following changes to `apps/data-service/src/jobs/queue.ts`:

```typescript
// At the top of src/jobs/queue.ts:
import * as Sentry from "@sentry/cloudflare";

// In the handleJobsBatch error catch block:
export async function handleJobsBatch(
  ...
  for (const message of batch.messages) {
    try {
      ...
      // Capture exception with Sentry including jobType and jobId tags
...
### B. Cron Task Sentry Integration

Currently, `cronTask` in `apps/data-service/src/jobs/cron.ts` logs task errors and rethrows them. In the test environment, the Sentry wrapper is bypassed because of `process.env.VITEST`.

We propose capturing the exception explicitly before rethrowing:

```typescript
// At the top of src/jobs/cron.ts:
import * as Sentry from "@sentry/cloudflare";

// In the cronTask error catch block:
async function cronTask(name: string, fn: () => Promise<void>): Promise<void> {
  ...
  try {
    ...
    // Capture exception with Sentry including cronTask tag
    Sentry.captureException(error, {
      tags: {
        cronTask: name,
      },
...
## 3. Proposed Integration Tests: `apps/data-service/src/sentry.test.ts`

The integration tests will mock `@sentry/cloudflare` using Vitest's `vi.mock` and intercept `captureException` calls. It leverages a mock D1Database to induce database execution failures, simulating real-world failures during Outbox / Queue processing and Scheduled Tick execution.
...
The integration tests can be run via the workspaces toolchain:

```bash
vp test run
```

This runs Vitest on all suites under `apps/data-service`.
