...
OpenRouter routes to hundreds of models across providers. Model IDs use the format
`provider/model-name`:
...
- Uses `camelCase` for option names (e.g., `topP`, `frequencyPenalty`), unlike OpenAI's
  `snake_case`.
