...
description: >-
  ...
  ToOptions/NavigateOptions/LinkOptions, from/to relative navigation, activeOptions/activeProps,
  preloading (intent/viewport/render), preloadDelay, navigation blocking (useBlocker, Block),
  createLink, linkOptions helper, scroll restoration, MatchRoute.
type: sub-skill
...
Use `useNavigate` only for side-effect-driven navigation (e.g., after a form submission). For
anything the user clicks, prefer `Link`.
...
Preloaded data stays fresh for 30 seconds by default (`defaultPreloadStaleTime: 30_000`). During
that window it won't be refetched. When using an external cache like TanStack Query, set
`defaultPreloadStaleTime: 0` to let the external library control freshness.
...
`linkOptions` provides eager type-checking on navigation options objects, so errors surface at
definition, not at spread-site:
...
Dynamic segments are declared with `$` in the route path. Always pass them via `params`. This
applies to `Link`, `useNavigate`, `Navigate`, and `router.navigate`.
...
Use `useNavigate` only for programmatic side-effect navigation (after form submit, async action,
etc).
...
Without `from`, only absolute paths are autocompleted and type-safe. Relative paths like `..`
resolve from root instead of the current route.
...
When you pass `search` as a plain object, it replaces all search params. Use the function form to
spread previous params and selectively update.

---

## Cross-References

- See also: **router-core/search-params/SKILL.md** — Link `search` prop interacts with search param
  validation
