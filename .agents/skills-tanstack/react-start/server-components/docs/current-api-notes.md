...
Validated against official TanStack Start, TanStack Router, TanStack blog, and Vite docs on
2026-04-13.
...
Important because some current RSC docs snippets still show the older `.validator(...)` spelling.
Normalize those examples before copying them into real code.
...
You may still find older official repo examples using `renderRsc` and older config shapes. Do not
cargo-cult them into new code. Normalize to the current docs and current `@tanstack/react-start/rsc`
APIs.

### Old validation snippets

If you see `.validator(z.object(...))` in an RSC example, rewrite it to `.validator(z.object(...))`
to match the current server function API.

### Old Vite config shapes

If an example enables Start but does not also install and register `@vitejs/plugin-rsc`, treat it as
stale for current TanStack Start RSC setup.
