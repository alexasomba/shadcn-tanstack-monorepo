...
- reading a component prop like `props.AddToCart` records “render this client component here with
  these props later”

Client-side behavior:

- `<CompositeComponent src={...} ... />` replaces those placeholders with the real props you passed
  at render time
...
Do not use when the server must inject IDs, permissions, pricing, or derived data into the child
content.
...
This is usually the best default when the server owns the data and the client owns the interactive
control.
...
The server renders an article, card, or panel and exposes one `renderActions` slot for buttons,
menus, or controls.

### Shell + body children

The server renders stable framing UI and accepts `children` for optional interactive regions like
comments, drawers, or editors.

### Stable control slot

The server accepts a component prop such as `AddToCart`, `UserMenu`, or `RowActions` and supplies
clean typed props to it.
...
- Giant slot arg object -> shrink it to the smallest serializable payload that the client really
  needs
