...
name: react-db
description: >
  React bindings for TanStack DB. useLiveQuery hook with dependency arrays (8 overloads: query
  function, config object, pre-created collection, disabled state via returning undefined/null).
  useLiveSuspenseQuery for React Suspense with Error Boundaries (data always defined).
  useLiveInfiniteQuery for cursor-based pagination (pageSize, fetchNextPage, hasNextPage,
  isFetchingNextPage). usePacedMutations for debounced React state updates. Return shape: data,
  state, collection, status, isLoading, isReady, isError. Import from @tanstack/react-db (re-exports
  all of @tanstack/db).
type: framework
library: db
framework: react
...
This skill builds on db-core. Read it first for collection setup, query builder, and mutation
patterns.
...
`@tanstack/react-db` re-exports everything from `@tanstack/db`. In React projects, import everything
...
When a query uses includes (subqueries in `select`), each child field is a live `Collection` by
default. Subscribe to it with `useLiveQuery` in a subcomponent:
...
See db-core/live-queries/SKILL.md for full includes rules (correlation conditions, nested includes,
aggregates).
...
When the query uses external state not in the deps array, the query won't re-run when that value
changes, showing stale results.
...
`useLiveSuspenseQuery` throws errors during rendering. Without an Error Boundary, the entire app
crashes.
...
If `useLiveQuery` throws
`InvalidSourceError: The value provided for alias "todo" is not a Collection`, it usually means two
copies of `@tanstack/db` are installed. The collection was created by one copy, but `useLiveQuery`
checks `instanceof` against the other.
...
The root cause is typically a dependency that bundles its own copy instead of declaring
`@tanstack/db` as a `peerDependency`.

### HIGH Tension: Query expressiveness vs. IVM constraints

The query builder looks like SQL but has constraints that SQL doesn't — equality joins only, orderBy
required for limit/offset, no distinct without select. Agents write SQL-style queries that violate
these constraints. See db-core/live-queries/SKILL.md § Common Mistakes for all constraints.
