...
TanStack DB accepts any [StandardSchema](https://standardschema.dev)-compatible library via the
`schema` option.
...
When a schema transforms A to B, TInput **must** accept both A and B. During `update()`, the draft
contains TOutput data.
...
**Schemas validate client mutations only** (`insert()`, `update()`). Synced data from backends
(Electric, PowerSync, etc.) bypasses the schema entirely.
...
With Zod, `z.coerce.date()` is simpler than the `z.union([z.string(), z.date()]).transform(...)`
pattern:
...
This satisfies the TInput-superset-of-TOutput requirement automatically. Other StandardSchema
libraries have their own coercion patterns — consult library docs.
