...
- `rollback()` allowed in `pending` or `persisting` (throws
  `TransactionAlreadyCompletedRollbackError` if completed)
...
When multiple mutations target the same item (same `globalKey`) within a transaction, they merge:
...
`(delete, update)` and `(delete, insert)` cannot occur -- the collection prevents operations on
deleted items within the same transaction.
...
Inside `tx.mutate(() => { ... })`, the transaction is pushed onto an internal stack. Any
`collection.insert/update/delete` call automatically joins the topmost ambient transaction. This is
how `createOptimisticAction` and `createPacedMutations` wire collection operations into their
transactions.
...
Rapid calls merge into the active transaction (via `applyMutations`) until the strategy fires the
commit. A new transaction is created for subsequent calls.
...
Queue creates a **separate transaction per call** (unlike debounce/throttle which merge). Each
transaction commits and awaits `isPersisted` before the next starts. Failed transactions do not
...
The promise is a `Deferred` -- it is created at transaction construction time and settled when
`commit()` completes or `rollback()` is called. For `autoCommit: true` transactions, the promise
settles shortly after `mutate()` returns (the commit runs asynchronously).
