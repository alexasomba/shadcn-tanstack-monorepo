...
- normalize to `renderServerComponent`, `createCompositeComponent`, `CompositeComponent`,
  `.validator(...)`
...
- passing classes, functions, Maps/Sets with custom serialization expectations, or other
  non-Flight-friendly values
...
- dev-mode import protection warnings can be informational because there is no tree-shaking; build
  output is the authoritative check
