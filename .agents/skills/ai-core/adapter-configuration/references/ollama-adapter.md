...
Ollama runs models locally. The adapter supports a large catalog of models. Key families include:
...
Ollama models use a generic options type. Provider options vary by the underlying model. The adapter
passes options through to the Ollama API. Sampling options are **nested** under
`modelOptions.options` (this matches Ollama's own request shape) — `temperature`, `top_p`, and
`num_predict` (max output tokens) all live there.
...
- Ollama requires models to be downloaded first (`ollama pull <model>`). The adapter does not
  auto-download models.
- The model catalog is very large (60+ model families). Model names follow Ollama's naming:
  `family:variant` (e.g., `llama3.3:70b`).
- Vision models (e.g., `llama3.2-vision`, `llava`, `gemma3`) support image input. Text-only models
  do not.
