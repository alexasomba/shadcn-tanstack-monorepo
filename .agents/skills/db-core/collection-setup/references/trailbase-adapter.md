...
TrailBase uses different data formats (e.g. Unix timestamps). Use `parse` and `serialize` for
...
Automatic when `enable_subscriptions` is enabled on the TrailBase server. No additional client
config needed -- the collection subscribes to changes automatically.
...
TrailBase handles persistence through the Record API automatically. Custom handlers are for
additional logic only.
