...
- `refetch(opts?)` -- manual refetch; `opts.throwOnError` (default `false`); bypasses
  `enabled: false`
...
Query predicates (where, orderBy, limit, offset) passed to `queryFn` via
...
When using a function-based `queryKey`, all derived keys must share the base key (`queryKey({})`) as
a prefix. TanStack Query uses prefix matching for cache operations; if derived keys don't share the
base prefix, cache updates silently miss entries, leading to stale data.
