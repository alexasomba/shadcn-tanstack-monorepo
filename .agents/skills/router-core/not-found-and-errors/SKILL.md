...
description: >-
  notFound() function, notFoundComponent, defaultNotFoundComponent, notFoundMode (fuzzy/root),
  errorComponent, CatchBoundary, CatchNotFound, isNotFound, NotFoundRoute (deprecated), route
...
TanStack Router handles two categories of "not found": unmatched URL paths (automatic) and missing
resources like a post that doesn't exist (manual via `notFound()`). Error boundaries are configured
per-route via `errorComponent`.

> **CRITICAL**: Do NOT use the deprecated `NotFoundRoute`. When present, `notFound()` and
> `notFoundComponent` will NOT work. Remove it and use `notFoundComponent` instead. **CRITICAL**:
> `useLoaderData` may be undefined inside `notFoundComponent`. Use `useParams`, `useSearch`, or
> `useRouteContext` instead.
...
Throw `notFound()` in `loader` or `beforeLoad` when a resource doesn't exist. It works like
`redirect()` — throw it to trigger the not-found boundary.
...
The router finds the nearest parent route with children and a `notFoundComponent`. Preserves as much
parent layout as possible.
...
`errorComponent` receives `error`, `info`, and `reset` props. For loader errors, use
`router.invalidate()` to re-run the loader — it automatically resets the error boundary.
...
`notFoundComponent` cannot reliably use `useLoaderData` because the loader may not have completed.
Safe hooks:
...
Route masking shows a different URL in the browser bar than the actual route being rendered. Masking
data is stored in `location.state` and is lost when the URL is shared or opened in a new tab.
...
Only routes with children (and therefore an `<Outlet>`) can render `notFoundComponent`. Leaf routes
(routes without children) will never catch not-found errors — the error bubbles up to the nearest
...
Masking data lives in `location.state` (browser history). When a masked URL is copied, shared, or
opened in a new tab, the masking data is lost. The browser navigates to the visible (masked) URL
directly.
...
- **router-core/data-loading** — `notFound()` thrown in loaders interacts with error boundaries and
  loader data availability. `errorComponent` retry requires `router.invalidate()`.
