...
- **Auth plugins** (server, shared D1): organization, **referral**, **admin**, **better-inbox**. Client split: **user-web** = org + referral + inbox; **admin-web** = org + admin + inbox. Referral product UX on user-web; admin has read-only `/referrals` stats. Inbox: `<InboxButton />` on both apps; producers use `createInboxNotification` (D1, not HTTP). Bootstrap admins: `role='admin'` or `BETTER_AUTH_ADMIN_USER_IDS`. After plugin changes: `vpr auth:generate` → `vpr db:generate` → `vpr db:migrate:local`.
